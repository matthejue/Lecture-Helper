...
local function set_commands()
  ...
  vim.api.nvim_create_user_command("ConvertLinesToNodes", function(opts)
    actions.convert_lines_to_nodes(opts.line1, opts.line2)
  end, { desc = state.descs.convert_lines_to_nodes, range = true })
...
local function set_global_keybindings()
  ...
  if state.opts.keys.generate_pdf_and_open then
    vim.keymap.set(
      "n",
      state.opts.keys.generate_pdf_and_open,
      actions.generate_pdf_and_open,
      { silent = true, desc = state.descs.generate_pdf_and_open }
    )
  end
  if state.opts.keys.update_pdf then
    vim.keymap.set(
      "n",
      state.opts.keys.update_pdf,
      actions.update_pdf,
      { silent = true, desc = state.descs.update_pdf }
    )
  end
  if state.opts.keys.convert_line_to_node then
    vim.keymap.set(
      "n",
  ...
  if state.opts.keys.convert_lines_to_nodes then
    vim.keymap.set(
      "v",
      state.opts.keys.convert_lines_to_nodes,
      function()
        actions.convert_lines_to_nodes()
      end,
      { silent = true, desc = state.descs.convert_lines_to_nodes }
    )
  end
  if state.opts.keys.remove_disturbing_prefix then
    vim.keymap.set(
      "v",
      state.opts.keys.remove_disturbing_prefix,
      function()
        actions.remove_disturbing_prefix()
      end,
